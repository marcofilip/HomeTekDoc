...
const isTechnician = (req, res, next) => {
    ...
    return res
        .status(403)
        .json({ error: "Forbidden: Access restricted to technicians" });
};
...
app.post("/auth/google", async (req, res) => {
    ...
    try {
        ...
        db.get(
            "SELECT * FROM auth_users WHERE google_id = ?",
            [googleId],
            (err, userByGoogleId) => {
                if (err) {
                    ...
                    return res
                        ...
                        .json({
                            authenticated: false,
                            error: "Database error checking Google ID.",
                        });
                }
                ...
                if (userByGoogleId) {
                    ...
                    console.log(
                        `Google Login: User found by google_id ${googleId}`
                    );
                    req.session.user = {
                        id: userByGoogleId.id,
                        username: userByGoogleId.username,
                ...
                console.log(
                    `Google Login: User not found by google_id ${googleId}. Searching by email ${email}...`
                );
                db.get(
                    ...
                    (err, userByEmail) => {
                        ...
                        if (userByEmail) {
                            ...
                            console.log(
                                `Google Login: User found by email ${email}. Checking google_id...`
                            );
                            if (
                                ...
                                console.error(
                                    "Google Login Conflict: Email",
                                    email,
                                    "linked to google_id",
                                    userByEmail.google_id,
                                    "but attempting login with",
                                    googleId
                                );
                                return res
                                    .status(409)
                                ...
                                console.log(
                                    `Google Login: Linking email ${email} to google_id ${googleId}`
                                );
                                db.run(
                                    "UPDATE auth_users SET google_id = ? WHERE id = ?",
                                    [googleId, userByEmail.id],
                                    (err) => {
                                        if (err) {
                                            console.error(
                                                "Error updating user with google_id:",
                                                err
                                            );
                                            return res
                                                .status(500)
                                ...
                                console.log(
                                    `Google Login: User found by email ${email} and matching google_id ${googleId}`
                                );
                                req.session.user = {
                                    id: userByEmail.id,
                                    username: userByEmail.username,
                            ...
                            console.log(
                                `Google Login: No user found for email ${email}. Creating new user...`
                            );
                  ...
                  `;
                            db.run(
                                insertQuery,
                                [newUsername, name, email, googleId],
                                function (err) {
                                    if (err) {
                                        console.error(
                                            "Error inserting new Google user:",
                                            err
                                        );
                                        if (
                                            ...
                                            err.message.includes(
                                                "UNIQUE constraint failed: auth_users.username"
                                            )
                                        ) {
                                        ...
                                        if (
                                            err.message.includes(
                                                "UNIQUE constraint failed: auth_users.google_id"
                                            )
                                        ) {
                                    ...
                                    console.log(
                                        `Google Login: New user created with ID ${newUserId}`
                                    );
                                    req.session.user = {
                                        id: newUserId,
...
app.get("/auth/profile", auth, (req, res) => {
    ...
    db.get(
        "SELECT id, nome, email, citta, indirizzo, telefono, role FROM auth_users WHERE id = ?",
        [req.session.user.id],
        (err, user) => {
            if (err) {
                return res.status(500).json({ error: err.message });
...
app.get("/utenti/filtrati", (req, res) => {
    ...
    if (!role || !["cliente", "tecnico", "admin"].includes(role)) {
        return res
            .status(400)
            .json({ error: "Valid role parameter is required" });
    }
    ...
    db.get(
        ...
        (err, row) => {
            ...
            if (!row) {
                return res
                    .status(404)
                    .json({ error: `Utente ${id} non trovato` });
            }
    ...
    if (
        ...
        tecnicoDb.getTechniciansAdvanced(
            ...
            (err, technicians) => {
                if (err) {
                    console.error(
                        "Error retrieving advanced technicians:",
                        err
                    );
                    return res.status(500).json({ error: err.message });
                }
...
app.get("/tecnici/specializzazione/:spec", (req, res) => {
    ...
    tecnicoDb.getTechniciansBySpecialization(
        ...
        (err, technicians) => {
            if (err) {
                console.error(
                    "Error retrieving technicians by specialization:",
                    err
                );
                return res.status(500).json({ error: err.message });
            }
...
app.post("/assistenza", auth, (req, res) => {
    ...
    if (!description || !technician_id || !title) {
        return res
            .status(400)
            .json({
                error: "Descrizione, ID tecnico e titolo sono obbligatori",
            });
    }
  ...
  `;
    ...
    db.run(
        query,
        [customer_id, description, urgFlag, Number(technician_id), title],
        function (err) {
            if (err) {
                ...
                if (err.message.includes("FOREIGN KEY constraint failed")) {
                    return res
                        .status(400)
                        .json({ error: "ID Tecnico specificato non valido." });
                }
                return res
                    .status(500)
                    .json({
                        error:
                            "Errore nell'invio della richiesta: " + err.message,
                    });
            }
            res.status(201).json({
...
app.get("/assistenze", auth, (req, res) => {
    ...
    if (!customer_id) {
        return res
            .status(401)
            .json({
                error: "Autenticazione richiesta o ID utente non trovato in sessione",
            });
    }
    ...
    db.all(query, [customer_id], (err, rows) => {
        if (err) {
            ...
            return res
                .status(500)
                .json({
                    error: "Errore nel recupero delle richieste di assistenza.",
                });
        }
...
app.get("/tecnici/me/assistenze", auth, isTechnician, (req, res) => {
    ...
    db.get(
        "SELECT id FROM technicians WHERE auth_user_id = ?",
        [auth_user_id],
        (err, technicianProfile) => {
            if (err) {
                console.error("Error finding technician profile:", err);
                return res
                    .status(500)
                    .json({ error: "Errore nel trovare il profilo tecnico." });
            }
            if (!technicianProfile) {
                console.error(
                    `No technician profile found for auth_user_id: ${auth_user_id}`
                );
                return res
                    .status(404)
                    .json({
                        error: "Profilo tecnico non trovato per l'utente corrente.",
                    });
            }
            ...
            db.all(query, [technician_id], (err, rows) => {
                if (err) {
                    ...
                    return res
                        .status(500)
                        .json({
                            error: "Errore nel recupero delle richieste di assistenza assegnate.",
                        });
                }
...
app.put("/assistenze/:id/status", auth, isTechnician, (req, res) => {
    ...
    db.get(
        ...
        (err, technicianProfile) => {
            if (err) {
                console.error(
                    "Error finding technician profile for status update:",
                    err
                );
                return res
                    .status(500)
    ...
    `;
            ...
            db.run(
                updateQuery,
                [newStatus, Number(assistenzaId), technician_id],
                function (err) {
                    if (err) {
                    ...
                    if (this.changes === 0) {
                        return res
                            .status(404)
                            .json({
                                error: "Richiesta non trovata o non assegnata a questo tecnico.",
                            });
                    }

                    res.json({
                        message: `Stato della richiesta ${assistenzaId} aggiornato a \"${newStatus}\" con successo.`,
                    });
                }
            );
        }
